...
const verifyUser: OAuth2Strategy<SessionUser>["verify"] = async ({
	...
	const { email, name, locale, imageUrl } = await getProfile(request, tokens);
	...
	const userId = isFirstTime ? nanoid() : lastAuthenticatedEvent.userId;

	const [user, userAuthenticatedEvent] = await Promise.all([
		createOrUpdateUser({ id: userId, email, name, locale, imageUrl }),
